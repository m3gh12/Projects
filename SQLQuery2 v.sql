...
select location, population, max(total_cases) as highest_infected, max(total_cases/population*100) AS pc_infected
from Portsql..c19d
...
--by continent
select continent,sum(distinct(cast(total_tests as bigint))) as tt,avg(cast(positive_rate as float)) as pr
from Portsql..c19vax
where continent is not null
group by continent
order by pr desc
...
--by countries
select location,sum(distinct(cast(total_tests as bigint))) as tt,avg(cast(positive_rate as float)) as pr
from Portsql..c19vax
group by location
order by pr desc
...
select dt.continent,dt.location,dt.date,dt.population,sum(convert(int,vx.new_vaccinations)) over (partition by dt.location order by dt.location,dt.date) as rolling_count_vax
from Portsql..c19d dt
join Portsql..c19vax vx 
on dt.location=vx.location
and dt.date=vx.date
...
as
(
select dt.continent,dt.location,dt.date,dt.population,sum(convert(bigint,vx.new_vaccinations)) over (partition by dt.location order by dt.location,dt.date) as rolling_count_vax
from Portsql..c19d dt
join Portsql..c19vax vx 
    on dt.location=vx.location
    and dt.date=vx.date
